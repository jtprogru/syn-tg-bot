...
async def random_number_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Срабатывает только при наличии слова 'рандом' в сообщении."""

    text = update.message.text
    if "рандом" in text.lower():
        # Ответ случайным числом, если в сообщении есть слово 'рандом'
        await context.bot.send_message(
            chat_id=update.effective_chat.id,
            text=f"Случайное число: {str(random.randint(1, 100))}",
        )
...
def main():
    ...
    application.add_handler(
        MessageHandler(
            filters.TEXT & (~filters.COMMAND) & filters.Regex("(?i)рандом"),
